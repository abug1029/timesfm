...
3. **机制层**：发现两个致命理论隐患
   ...
   - **级联架构的日内前视偏差（已确认存在，必须修复）**

**关键发现**：当前回测系统存在严重的前视偏差（lookahead bias），所有日内评估点都使用了"未来"的日线数据。这是一个必须立即修复的致命缺陷。
...
|------|------|---------|---------|---------|---------|------|
| 当前 | STEP=24 | 396 | 7-8 年 | 每周 1-2 次 | **存在** | 2.1h |
| 第一次修订 | STEP=16 | 594 | 7-8 年 | 每 3 天 1 次 | **存在** | 3.1h |
| **第二次修订** | **STEP=2** | **600** | **200 交易日** | **每天 3 次** | **修复后无** | **3.1h** |
...
- ⏳ 修复前视偏差（`BacktestDataStore.get_main_continuous`）
- ⏳ 修改 STEP=24 → STEP=2
- ⏳ 新增 EVAL_WINDOW_BARS=1200
- ⏳ 修改评估窗口截断逻辑
- ⏳ 重新运行慢环评估
