...
```python
    ...
    precomputed = {
        "hurst": calc_rolling_hurst(closes_full, window=120, step=6),          # np.ndarray, 长度=len(closes_full), [t_idx] 安全
    ...
    if "open_interest" in df_1h.columns and df_1h["open_interest"].notna().any():
        precomputed["oi_pct_change"] = calc_oi_pct_change(df_1h["open_interest"]).values  # pd.Series->np.ndarray
    # daily_slope: 按 valid 字典映射 (key=t_idx), 防 IndexError
    # hurst/hourly_slope/rsi_state/oi_pct_change 是全序列数组, [t_idx] 用绝对索引安全;
    # 但 daily_slope 按 valid 推导 (长度=len(valid)=396), 用数组 [t_idx=480] 会越界 -> 字典 O(1) 查找
