...
| `test_supervisor.py` | 三环监督环（harvest / 429 / cycle / known_verdicts 线索 / 评分） |
| `test_harvest_proposals.py` | 方案 A 提案收割（机制校验 / DEAD-HOLD / failure_delta / 去重 / 选座） |
| `test_fm_two_peer_topology.py` | cohort_size=2 与 `fm_two_peer` rotation 对账 |
...
python -m pytest tests/test_supervisor.py tests/test_harvest_proposals.py \
  tests/test_fm_two_peer_topology.py tests/test_praxist_evidence_ladder.py \
