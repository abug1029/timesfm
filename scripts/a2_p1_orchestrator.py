"""A2-P1 Orchestrator: 调度 Worker 子进程, 品种级隔离 + 断点续跑 + run manifest。

Usage:
    python scripts/a2_p1_orchestrator.py                          # 全 20 品种, run_id=a2-p1.1
    python scripts/a2_p1_orchestrator.py --run-id a2-p1           # 用 a2-p1 配置
...
def run_orchestrator(symbols: list, force: bool = False, run_id: str = "a2-p1.1"):
...
def _run_orchestrator_core(
    ...
    if all_ok and per_symbol_status:
        ...
        if report_script.exists():
            subprocess.run([python, str(report_script), "--run-id", run_id], cwd=FM_ROOT)
    else:
        bad = [
