"""KQ.i@m 指数合约日线回填/增量同步 (spec: docs/2026-09-18-oi-gated-momentum-spec.md §3.1)

指数合约 open_interest = 全市场总持仓 —— 杜绝主力换月污染 (主力单合约口径 ΔOI_5d max=154%)。

用法:
  python scripts/fetch_index_continuous.py                 # 全量回填 (2016-01-05 → 今)
...
from data.tqsdk_fetcher import TqSdkFetcher  # noqa: E402

SYMBOL = "m"
TQ_INDEX_SYMBOL = "KQ.i@DCE.m"
START_DATE = "2016-01-05"
FIRST_DATE_HARD_LIMIT = "2016-03-01"
...
def compute_quality(df: pd.DataFrame, calendar: list) -> dict:
    """回退线判定 + 质量统计。df 需升序, dt 已过滤 >= START_DATE。"""
    dates = df["dt"].tolist()
    date_set = set(dates)
...
def rollback_verdict(q: dict) -> list:
    """回退触发线判定。返回触发的检查列表 (空 = 通过)。"""
    triggered = []
    if q["first_valid_date"] is None:
        triggered.append("no_data")
    elif q["first_valid_date"] > FIRST_DATE_HARD_LIMIT:
        triggered.append(
            f"first_valid_date {q['first_valid_date']} > {FIRST_DATE_HARD_LIMIT} "
            f"(KQ.i@m 历史深度不足, 须回退逐合约回填方案)"
        )
...
def fetch_df(fetcher: TqSdkFetcher) -> pd.DataFrame:
...
def main() -> int:
    parser = argparse.ArgumentParser(description="KQ.i@m 指数合约日线回填/增量同步")
